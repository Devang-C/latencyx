...
class LatencyXTail:
    ...
    def run(self):
        ...
        if not self.file_path.exists():
            ...
            print("   Make sure 'json_file' exporter is enabled in your LatencyX config")
            return
    ...
    def _print_compact_row(self, data):
        ...
        parts = [f"[{span_type}]", span_name, f"duration={duration_str}"]
...
Examples:
  ...
  latencyx tail --no-follow
    ...
    if args.command == "tail":
        tailer = LatencyXTail(file_path=args.file, follow=not args.no_follow, format=args.format)
        tailer.run()
    else:
        parser.print_help()
